        ...
        for arg in args:
            if isinstance(arg, LineRoot):
                ...
                try:
                    ...
                    print(
                        "[lineiterator.py] Not a LineNum and is not a LineSeries -"
                        f" bail out; \nException: {e}"
                    )
...
class DataAccessor(LineIterator):
    ...
    PriceOpenInterest = DataSeries.OpenInterest
    PriceDateTime = DataSeries.DateTime
    PriceCumulativeClose = DataSeries.CumulativeClose
    PriceCumulativeLow = DataSeries.CumulativeLow
